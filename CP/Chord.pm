...
#
# Returns a Chord object whose elements are:
#   Array of the component parts of the chord.
#   A string made of the above components.
#   A string containing any non-chord text.
# (Removes any parentheses G#(maj7) -> G#maj7 )
#
sub new {
  my($proto,$c) = @_;
  ...
  if ($c =~ /^\*/) {
    ($self->{text} = $c) =~ s/^\*//;
  #                 1         2      3               4         5      6             7
  } elsif ($c =~ /^([A-G]{1})([#b]?)([\(\)a-z\d]*)\/([A-G]{1})([#b]?)([\(\)a-z\d]*)(\s*.*)/) {
    # Find 'C/G text' with any variants to the split chord components
    $self->{bits}  = [$1, $2, $3, '/', $4, $5, $6];
    $self->{chord} = $1.$2.$3.'/'.$4.$5.$6;
    $self->{text}  = $7;
    $self->{bits}[2] =~ s/[()]//g;
    $self->{bits}[6] =~ s/[()]//g;
  #                 1         2      3             4
...
# Transpose a Chord object returning a string.
# This does not alter the underlying chord.
sub trans2str {
  my($self,$pro) = @_;

  my $ch = "";
  my $bits = $self->{bits};
  if ($KeyShift && $bits->[0] =~ /^[A-G]/) {
    my($a,$b) = _tr($pro,$bits->[0],$bits->[1]);
    if (@{$bits} > 1) {
      $ch = $a.$b.$bits->[2];
      if (@{$bits} > 3) {
	($a,$b) = _tr($pro,$bits->[4],$bits->[5]);
	$ch .= '/'.$a.$b.$bits->[6];
      }
    } else {
      $ch = $a;
...
# Transpose a copy of a Chord object and return it.
sub trans2obj {
  my($self,$pro) = @_;

  if ($KeyShift && @{$self->{bits}}) {
    my $copy = {};
    bless $copy, "CP::Chord";
    foreach (@{$self->{bits}}) {
      push(@{$copy->{bits}}, $_);
    ...
    if (@{$copy->{bits}} > 1) {
      ($copy->{bits}[0],$copy->{bits}[1]) = _tr($pro,$copy->{bits}[0],$copy->{bits}[1]);
      if (@{$self->{bits}} > 3) {
	($copy->{bits}[4],$copy->{bits}[5]) = _tr($pro,$copy->{bits}[4],$copy->{bits}[5]);
      }
    }
    $copy->{chord} = join('', @{$copy->{bits}});
    $copy->{text} = $self->{text};
    return $copy;
  } else {
    return $self;
